...
def MERGE_SORT_21(A: list[int], p: int, k: int) -> None:
    if p < k:
        s = int((k - p)/3)*2 + p
...
def MERGE_SORT_21_PLUS(A: list[int], p=0, k=0, full_size=False) -> tuple[int, int]:
    if full_size:
        p, k = 0, len(A)-1
    if p < k:
        s = int((k - p)/3)*2 + p
        comp_1, rewr_1 = MERGE_SORT_21_PLUS( A, p, s )
        comp_2, rewr_2 = MERGE_SORT_21_PLUS( A, s+1, k )
